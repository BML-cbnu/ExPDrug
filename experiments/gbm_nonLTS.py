...
""" GBM : target_class = 1 : LTS  // 0 : LTS failed """
...
def get_config():
    config = {
        ...
        "file_paths": {
            ...
            "model_save": '/home/bml_jk/py/lrpGP/savedModel/GBM/lts_failed_valid.pth',
            "result_save": '/home/bml_jk/py/lrpGP/result/gbm_LTS/valid_LTS_failed_pathway_info.csv'
        },
        "params": {
...
class ModelTrainer:
    ...
    def train(self, logger, verbose=True):
        ...
        # Select the model where Precision is closest to the average Precision
        precisions = [result[2] for result in fold_results]  # Extract Precision scores
        avg_precision = np.mean(precisions)  # Calculate the average Precision
        closest_index = np.argmin([abs(precision - avg_precision) for precision in precisions])  # Find the index of Precision closest to the average
        best_model_state_dict = fold_model_state_dicts[closest_index]  # Select the model with Precision closest to the average
        ...
        if self.save_model and best_model_state_dict is not None:
            ...
            logger.info(f"Saved median model from fold {closest_index + 1} to {self.save_model_path}")
...
class p_Test:
    ...
    def run_permutation_test(self, K, gene_pathway_matrix, h1_to_pathway_masking, target_class=0):
    ...
    def compute_lrp_scores(self, model, dataset, target_class=0, num_iterations=100):
...
class LRPHandler:
    ...
    @staticmethod
    def compute_lrp_for_dataset_multiple_times(model, dataset, batch_size=32, num_iterations=1000, target_class=0):
...
if __name__ == "__main__":
    ...
    obtained_lrp_scores = LRPHandler.compute_lrp_for_dataset_multiple_times(kFold_best_model, dataset, target_class=0)
    ...
    all_lrp_scores = ptest_trainer.run_permutation_test(K, gene_pathway_matrix, h1_to_pathway_masking, target_class=0)
